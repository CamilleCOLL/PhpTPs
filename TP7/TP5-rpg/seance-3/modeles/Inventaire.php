...
class Inventaire
{
    ...
    public function __construct($proprietaire, $emplacementsMax = 10) {
        if (empty(trim($proprietaire))) {
            throw new Exception("Le propriétaire ne peut pas être vide.");
        }
        if (!is_int($emplacementsMax) || $emplacementsMax < 1) {
            throw new Exception("Le nombre d'emplacements doit être un entier positif.");
        }
        $this->proprietaire    = trim($proprietaire);
        $this->emplacementsMax = $emplacementsMax;
    }
